...
<div class="row g-4">
    <div class="col-lg-6">
        <div class="panel">
            <div class="panel-header">
                <span class="panel-title">✏️ Modifier mon profil</span>
            </div>
    ...
    <div class="col-lg-6">
        <div class="panel">
            <div class="panel-header">
                <span class="panel-title" style="color: #ef4444;">⚠️ Zone dangereuse</span>
            </div>
            <div class="panel-body">
                ...
                <form method="POST" action="" 
                    ...
                    <button type="submit" name="delete_account" class="btn-danger-ecall w-100">
        ...
        <div class="panel mt-3">
            <div class="panel-header">
                <span class="panel-title">📊 Mes statistiques</span>
            </div>
            <div class="panel-body">
                <?php
                $stmt = $pdo->prepare("SELECT COUNT(*) as total, SUM(duree) as total_duree FROM appels WHERE agent_id = ? OR agent_id IS NULL");
                $stmt->execute([$_SESSION['user_id']]);
                $stats = $stmt->fetch();
                ?>
                <p><strong>Appels traités :</strong> <?= $stats['total'] ?? 0 ?></p>
                <p><strong>Temps total :</strong> 
                    <?php 
                    if ($stats['total_duree']) {
                        $heures = floor($stats['total_duree'] / 3600);
                        $minutes = floor(($stats['total_duree'] % 3600) / 60);
